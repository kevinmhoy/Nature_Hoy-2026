...
A_ = []
mod_fixed_ = [None, None]
cornums = None

logging.getLogger('emcee').setLevel('CRITICAL')

# BEGIN WRITE_DATA_RV FROM MODEL
my_data = pd.read_csv('datalogs/CD352722/run_5/k0/temp/temp_data.csv', index_col=0)
...
def my_prior(theta):
    ...
    lp += Uniform(theta[0], [-150, 30], -5.19295685089021)

    if lp == -np.inf:
        return lp


    lp += Normal(theta[1], [0, 60], [0, 10, np.float64(-0.6931471825331207)])
...
ndim = 2
betas = [1.0, 0.5766, 0.3431, 0.2041, 0.1177, 0.06451, 0.0345, 0.01789, 0.007544, 0.002111]
progress = True
