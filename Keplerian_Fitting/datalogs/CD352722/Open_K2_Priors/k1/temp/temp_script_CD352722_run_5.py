...
A_ = []
mod_fixed_ = [None, None, None, None, None, None, None]
cornums = None

logging.getLogger('emcee').setLevel('CRITICAL')

# BEGIN WRITE_DATA_RV FROM MODEL
my_data = pd.read_csv('datalogs/CD352722/run_5/k1/temp/temp_data.csv', index_col=0)
...
def my_model(theta):
    ...
    model0 += theta[slice(5, 6, None)][1-1] * mask1

    # Instrumental Part, jitter00.model
    
    err20 += mask1 * theta[slice(6, 7, None)][1-1] ** 2
...
def my_prior(theta):
    ...
    lp += Uniform(theta[5], [-150, 30], -5.19295685089021)

    if lp == -np.inf:
        return lp


    lp += Normal(theta[6], [0, 60], [0, 10, np.float64(-0.6931471825331207)])
...
ndim = 7
betas = [np.float64(1.0), np.float64(0.5003842551072356), np.float64(0.2981474102069325), np.float64(0.1944260055890018), np.float64(0.12897020164761056), np.float64(0.08311783183140774), np.float64(0.05168862237671318), np.float64(0.030628649732862064), np.float64(0.01604531229053516), np.float64(0.002111)]
progress = True
